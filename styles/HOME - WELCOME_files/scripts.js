...
function asd(elem) {
	...
	var otherChild = parent.lastElementChild;


	if(txt != "") {
		otherChild.className = "pined";
	} else {
		otherChild.className = "onField";
	}
}
...
function showDiv(popup, inpopup) {
	//var popup = document.getElementById('popup');
	//var inpopup = document.getElementById('popup_inside');
	var x = event.clientX;
    var y = event.clientY;

    inpopup.style.opacity = "1";
	popup.style.display = "block";
	popup.style.visibility = "visible";
	var z = document.getElementById('popup_inside2');
	if(z == null) {
		inpopup.style.left = x + "px";
		inpopup.style.top = (y - 90) + "px";
	}/* else {
...
function destroyElem(destroy, hidden) {
	...
	if(text != "") {
		...
		if(submitButton == null) {
			...
			changingForm.setAttribute('method', 'GET');
		...
		if(title == null && hidden == '1') {
			...
			hidden1.setAttribute('value', text);
			hidden1.setAttribute('name', 'title');
			hidden1.setAttribute('id', 'title');

			changingForm.appendChild(hidden1);
		} else if(hours == null && hidden == '2') {
			hidden1.setAttribute('type', 'hidden');
			hidden1.setAttribute('value', text);
			hidden1.setAttribute('name', 'hours');
			hidden1.setAttribute('id', 'hours');

			changingForm.appendChild(hidden1);
		} else if(task == null && hidden == '3') {
			hidden1.setAttribute('type', 'hidden');
			hidden1.setAttribute('value', text);
			hidden1.setAttribute('name', 'task');
			hidden1.setAttribute('id', 'task');

			changingForm.appendChild(hidden1);
		} else if(reflection == null && hidden == '4') {
			hidden1.setAttribute('type', 'hidden');
			hidden1.setAttribute('value', text);
			hidden1.setAttribute('name', 'reflection');
			hidden1.setAttribute('id', 'reflection');

			changingForm.appendChild(hidden1);
		} else if(title != null && hidden == '1') {
			title.value = text;
		} else if(hours != null && hidden == '2') {
			hours.value = text;
		} else if(task != null && hidden == '3') {
